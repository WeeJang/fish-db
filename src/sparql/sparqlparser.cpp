...
{
	return TriplePatternElem(TPElem::Type::Variable,TPElem::LiteralType::None, variable);
}
...
{
	return TriplePatternElem(TPElem::Type::Literal, TPElem::LiteralType::None, literal);
}

SPARQLParser::TPElem SPARQLParser::make_integer_literal(std::string literal)
{
	return TriplePatternElem(TPElem::Type::Literal, TPElem::LiteralType::Integer, literal);
}

SPARQLParser::TPElem SPARQLParser::make_double_literal(std::string literal)
{
	return TriplePatternElem(TPElem::Type::Literal, TPElem::LiteralType::Double, literal);
}
...
SPARQLParser::TPElem SPARQLParser::make_string_literal(std::string literal)
{
	...
	return TriplePatternElem(TPElem::Type::Literal, TPElem::LiteralType::String, cleaned_literal);
}

SPARQLParser::TPElem SPARQLParser::make_function_literal(std::string literal)
{
	return TriplePatternElem(TPElem::Type::Literal, TPElem::LiteralType::Function, literal);
}


//---------------------------------

std::string SPARQLParser::to_string(const SPARQLParser::TPElem& triple_pattern_elem)
{
...
std::string SPARQLParser::to_string(const SPARQLParser::TriplePattern& triple_pattern)
{
	std::stringstream sstream;
	if(triple_pattern.is_option_)
		sstream << "option" ;
...
std::string SPARQLParser::to_string(const SPARQLParser::Filter& filter)
{
	std::stringstream sstream;
	sstream << "filter";
	sstream << "(" << filter.variable_str_ \
		    << " " << filter.filter_type_ ;
	for(auto it = filter.filter_args_.begin(); it != filter.filter_args_.end();it ++)
	{
		//sstream << " " << *it ;
...
std::string SPARQLParser::to_string()
{
	...
	for(auto it = triple_patterns_.begin(); it != triple_patterns_.end() ; \
			it ++)
	{
...
void SPARQLParser::parseFilter()
{
	...
	std::cout << "filter str : " << sparql_lexer_.get_current_token_value() << std::endl;
	switch(token)
	{
