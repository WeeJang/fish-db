...
public:
	void add_vlv(const std::string &subj,const std::string &pred,const std::string &obj)
	{
	 	LOG("add_vlv : %s,%s,%s",subj.c_str(), pred.c_str(), obj.c_str());
	}

    void add_blv(const std::string &subj,const std::vector<std::string> &bound_vals,
	...
	//这里暴露出一个问题：像这样的接口（客户端call）能否建议山哥 使用 shared_ptr<Index::Bigpita::ValueFilter>.
	//不然对生命周期的管理不太方便
    void add_vlf(const std::string &subj, const std::string &pred,
                  const std::string &obj, Index::Bigpita::ValueFilter *fil)
	{
	 	LOG("add_vlf : %s,%s,%s,%s",subj.c_str(), pred.c_str(), obj.c_str(),fil->to_string().c_str());
	}   
    ...
    void add_opt_vlf(const std::string &subj, const std::string &pred,
                  const std::string &obj, Index::Bigpita::ValueFilter *fil)
	{
	 	LOG("add_opt_vlf : %s,%s,%s,%s",subj.c_str(), pred.c_str(), obj.c_str(), fil->to_string().c_str());
	}
...
private:
	...
	std::unique_ptr<ValueFilter> make_filter(const SPARQLParser::TriplePatternElem&);
public:
