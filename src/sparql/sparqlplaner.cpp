...
//--------------------------------------------------------
std::unique_ptr<ValueFilter> SPARQLPlaner::make_filter(
    const SPARQLParser::Filter& filter) {
  ...
  switch (filter.filter_type_) {
    ...
    case FilterType::Equal:
      ...
      if (filter_arg_0.literal_type_ == FilterArgsType::Double) {
        return sparql::make_unique<AmountFilter>(
            AmountFilter::Type::Equal, atof(filter_arg_0.value_.c_str()));
      }
      // like: ?comp_name = "IBM"
    ...
    case FilterType::NotEqual:
      if (filter_arg_0.literal_type_ == FilterArgsType::Double) {
        return sparql::make_unique<AmountFilter>(
            AmountFilter::Type::NotEqual, atof(filter_arg_0.value_.c_str()));
      }
      break;
    // like <
    // NOTE:当前仅支持数值型(double)
    case FilterType::Less:
      if (filter_arg_0.literal_type_ == FilterArgsType::Double) {
        return sparql::make_unique<AmountFilter>(
            AmountFilter::Type::Less, atof(filter_arg_0.value_.c_str()));
      }
      break;
    // like <=
    // NOTE:当前仅支持数值型(double)
    case FilterType::LessOrEq:
      if (filter_arg_0.literal_type_ == FilterArgsType::Double) {
        return sparql::make_unique<AmountFilter>(
            AmountFilter::Type::LessOrEq, atof(filter_arg_0.value_.c_str()));
      }
      break;
    // like >
    // NOTE:当前仅支持数值型(double)
    case FilterType::Greater:
      if (filter_arg_0.literal_type_ == FilterArgsType::Double) {
        return sparql::make_unique<AmountFilter>(
            AmountFilter::Type::Greater, atof(filter_arg_0.value_.c_str()));
      }
      break;
    // like >=
    // NOTE:当前仅支持数值型(double)
    case FilterType::GreaterOrEq:
      if (filter_arg_0.literal_type_ == FilterArgsType::Double) {
        return sparql::make_unique<AmountFilter>(
            AmountFilter::Type::GreaterOrEq, atof(filter_arg_0.value_.c_str()));
      }
      break;
...
//--------------------------------------------------------
std::unique_ptr<ValueFilter> SPARQLPlaner::make_filter(
    const SPARQLParser::TriplePatternElem& triple_elem) {
  ...
  switch (triple_elem.type_) {
    ...
    case TripleElemType::Literal:
      if (triple_elem.literal_type_ == TripleElemLiteralType::Double) {
        return sparql::make_unique<AmountFilter>(
            AmountFilter::Type::Equal, atof(triple_elem.value_.c_str()));
...
query::TripleSetQuery SPARQLPlaner::handle(const std::string sparql_query) {
  ...
  auto get_next_temporary_var_name = [&template_var, &current_var_count,
                                      &current_variables]() {
    while (true) {
  ...
  for (auto triple_it = triple_patterns.begin();
    ...
    case SPARQLParser::TriplePatternElem::Type::Variable:
      ...
      if (triple_it->obj_.type_ ==
          SPARQLParser::TriplePatternElem::Type::Variable) {
        std::string obj_str = triple_it->obj_.value_;
        auto obj_filter = filters.find(obj_str);

        if (obj_filter != filters.end()) {
          std::unique_ptr<ValueFilter> p_valuefilter =
              make_filter(obj_filter->second);
          if (p_valuefilter == nullptr) return {};
          if (triple_it->is_option_) {
            triple_set.add_opt_vlf(
                triple_it->subj_.value_, triple_it->pred_.value_,
          ...
          if (triple_it->is_option_) {
            triple_set.add_opt_vlv(triple_it->subj_.value_,
                                   triple_it->pred_.value_,
      ...
      // obj为literal
      // 则根据其数值类型，建立对应过滤器(即等号过滤器).在山哥kg内部IRI与str是一样的.
      else if (triple_it->obj_.type_ ==
               SPARQLParser::TriplePatternElem::Type::Literal) {
        std::unique_ptr<ValueFilter> p_valuefilter =
            make_filter(triple_it->obj_);
        if (p_valuefilter == nullptr) return {};
        std::string new_var = get_next_temporary_var_name();
        if (triple_it->is_option_) {
          triple_set.add_opt_vlf(triple_it->subj_.value_,
                                 triple_it->pred_.value_, new_var,
                                 p_valuefilter.release());
        } else {
          triple_set.add_vlf(triple_it->subj_.value_, triple_it->pred_.value_,
