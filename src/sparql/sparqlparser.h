...
public:
	...
	{
		std::string message_;
		ParserException(std::string message):message_(message){ LOG("%s",message_.c_str()); }
		ParserException(const char* message):message_(message){ LOG("%s",message); }
...
public:
	std::vector<std::string> project_variables() const { return project_variables_;}
	std::vector<TriplePattern> triple_patterns() const { return triple_patterns_;}
