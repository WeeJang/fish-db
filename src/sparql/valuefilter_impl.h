...
class AmountFilter : public Index::Bigpita::ValueFilter {
 public:
  ...
  AmountFilter(AmountFilter::Type type, double datum)
      : type_(type), datum_(datum) {
    LOG("AmountFilter Init");
  }
...
class StringFilter : public Index::Bigpita::ValueFilter {
 public:
  StringFilter(const std::set<std::string> datum_set) : datum_set_(datum_set) {
    LOG("StringFilter Init");
  }
  bool operator()(const Index::Bigpita::ReturnValue&) override;
  std::string to_string() const override;
...
class DateFilter : public StringFilter {
 public:
  DateFilter(std::set<std::string> datum_set) : StringFilter(datum_set) {
    LOG("DateFilter  Init");
  }
  std::string to_string() const override;
};
