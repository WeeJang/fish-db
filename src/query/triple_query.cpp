...
bool SharedQueryData::contains_var_in_bounds_vals_map(std::string& var_name) {
  ...
  std::cout << (hv_bound_vals_.find(var_name) != hv_bound_vals_.end())
            << std::endl;
  std::cout << (ss_bound_vals_.find(var_name) != ss_bound_vals_.end())
            << std::endl;
  printf_hv_bound_vals();
  printf_ss_bound_vals();
  return (hv_bound_vals_.find(var_name) != hv_bound_vals_.end()) ||
         (ss_bound_vals_.find(var_name) != ss_bound_vals_.end());
}
...
void SharedQueryData::printf_hv_bound_vals() {
  ...
  for (auto iter = hv_bound_vals_.begin(); iter != hv_bound_vals_.end();
       iter++) {
    std::cout << iter->first << ": (";
...
void SharedQueryData::printf_ss_bound_vals() {
  ...
  for (auto iter = ss_bound_vals_.begin(); iter != ss_bound_vals_.end();
       iter++) {
    std::cout << iter->first << ": (";
    for (auto& elem : iter->second) {
      std::cout << elem << ",";
...
void SharedQueryData::printf_intermediate_result() {
  ...
  for (auto iter = intermediate_result_.begin();
       iter != intermediate_result_.end(); iter++) {
    std::cout << "var: " << iter->first << std::endl;
    auto var_val_map = iter->second;
    for (auto val_ter = var_val_map.begin(); val_ter != var_val_map.end();
         val_ter++) {
...
void SharedQueryData::printf_intermediate_result_col_name() {
  ...
  for (auto iter = intermediate_result_col_name_.begin();
       iter != intermediate_result_col_name_.end(); iter++) {
...
int SharedQueryData::make_cartesian_product_by_filter_vector_linked(
    std::string another_var_name, std::string var_name,
    std::vector<std::string> (&filter_var_val_vector)[2]) {
  auto got_another_var_intermediate_result =
      intermediate_result_.find(another_var_name);
  auto got_var_intermediate_result = intermediate_result_.find(var_name);
  std::unordered_map<std::string, std::vector<std::string>>
      another_val_2_var_val_map;
  if (got_another_var_intermediate_result == intermediate_result_.end() &&
    ...
    for (size_t i = 0; i < filter_var_val_vector[0].size(); i++) {
      another_val_2_var_val_map[filter_var_val_vector[0][i]].push_back(
          filter_var_val_vector[1][i]);
    }
    intermediate_result_col_name_[another_var_name].push_back(var_name);
  } else if (got_another_var_intermediate_result ==
                 intermediate_result_.end() &&
             got_var_intermediate_result != intermediate_result_.end()) {
    ...
    for (size_t i = 0; i < filter_var_val_vector[0].size(); i++) {
      ...
      if (var_val_2_vec == var_map.end()) {
        ...
        for (auto& val : val_2_vec) {
          another_val_2_var_val_map[another_var_val].push_back(
              std::string(var_val).append("\t").append(val));
        }
      }
    }
    intermediate_result_col_name_[another_var_name].push_back(var_name);
    ...
    for (size_t i = 0; i < filter_var_val_vector[0].size(); i++) {
      ...
      if (another_var_val_2_vec == another_var_map.end()) {
        ...
        for (auto val : val_2_vec) {
          another_val_2_var_val_map[another_var_val].push_back(
              std::string(val).append("\t").append(var_val));
          ;
        }
      }
    }
    ...
    for (size_t i = 0; i < filter_var_val_vector[0].size(); i++) {
      ...
      auto var_val_2_vec = var_map.find(var_val);
      if (var_val_2_vec == var_map.end() ||
          another_var_val_2_vec == another_var_map.end()) {
        continue;
      } else {
        std::vector<std::string> val_vec = var_val_2_vec->second;
        std::vector<std::string> another_val_vec =
            another_var_val_2_vec->second;
...
TripleQuery::TripleQuery(HashValue sub, ShortString pre, Variable obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
...
TripleQuery::TripleQuery(HashValue sub, Variable pre, HashValue obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(sub.value());
...
TripleQuery::TripleQuery(HashValue sub, Variable pre, ShortString obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(sub.value());
  iri_vec_.push_back(obj.value());
  iri_pos_.push_back(core::TripleElemPos::sub);
...
TripleQuery::TripleQuery(HashValue sub, Variable pre, Variable obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(sub.value());
  iri_pos_.push_back(core::TripleElemPos::sub);
  spo_vec_iri_type_tag_[0] = IRITypeUnionTag::HV;
...
TripleQuery::TripleQuery(ShortString sub, ShortString pre, Variable obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(sub.value());
  iri_vec_.push_back(pre.value());
  iri_pos_.push_back(core::TripleElemPos::sub);
...
TripleQuery::TripleQuery(ShortString sub, Variable pre, HashValue obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(sub.value());
  iri_vec_.push_back(obj.value());
  iri_pos_.push_back(core::TripleElemPos::sub);
...
TripleQuery::TripleQuery(ShortString sub, Variable pre, ShortString obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(sub.value());
  iri_vec_.push_back(obj.value());
  iri_pos_.push_back(core::TripleElemPos::sub);
...
TripleQuery::TripleQuery(ShortString sub, Variable pre, Variable obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(sub.value());
  iri_pos_.push_back(core::TripleElemPos::sub);
  spo_vec_iri_type_tag_[0] = IRITypeUnionTag::SS;
...
TripleQuery::TripleQuery(Variable sub, ShortString pre, HashValue obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(pre.value());
  iri_vec_.push_back(obj.value());
  iri_pos_.push_back(core::TripleElemPos::pre);
...
TripleQuery::TripleQuery(Variable sub, ShortString pre, ShortString obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(pre.value());
  iri_vec_.push_back(obj.value());
  iri_pos_.push_back(core::TripleElemPos::pre);
...
TripleQuery::TripleQuery(Variable sub, ShortString pre, Variable obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(pre.value());
  iri_pos_.push_back(core::TripleElemPos::pre);
  spo_vec_iri_type_tag_[0] = IRITypeUnionTag::UD;
...
TripleQuery::TripleQuery(Variable sub, Variable pre, HashValue obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(obj.value());
  iri_pos_.push_back(core::TripleElemPos::obj);
  spo_vec_iri_type_tag_[0] = IRITypeUnionTag::UD;
...
TripleQuery::TripleQuery(Variable sub, Variable pre, ShortString obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  iri_vec_.push_back(obj.value());
  iri_pos_.push_back(core::TripleElemPos::obj);
  spo_vec_iri_type_tag_[0] = IRITypeUnionTag::UD;
...
TripleQuery::TripleQuery(Variable sub, Variable pre, Variable obj,
                         std::shared_ptr<SharedQueryData> p_shared) {
  spo_vec_iri_type_tag_[0] = IRITypeUnionTag::UD;
  spo_vec_iri_type_tag_[1] = IRITypeUnionTag::UD;
  spo_vec_iri_type_tag_[2] = IRITypeUnionTag::HV;
...
int TripleQuery::get_iri_index(IRITypeUnion_T iri_variant,
                               IRITypeUnionTag typetag, core::TripleElemPos pos,
                               BitMap_T& ret_bitmap) {
  if (IRITypeUnionTag::HV == typetag) {
    auto iri_value = boost::get<HV_T>(iri_variant);
    auto p_iri_index =
        p_shared_->p_fish_db_->hv_meta_iri_index_.get_IRI_index(iri_value);
    if (!p_iri_index) {
      return -1;
    } else {
    ...
    auto p_iri_index =
        p_shared_->p_fish_db_->ss_meta_iri_index_.get_IRI_index(iri_value);
    if (!p_iri_index) {
      return -1;
    } else {
      ret_bitmap = p_iri_index->at(pos);
...
void TripleQuery::init() {
  ...
  LOG("cur_valid_row_bm_index_cardinality : %d ",
      cur_valid_row_bm_index_cardinality_);
  is_valid_ = true;
}

//完善select 出来的vec的type tag
void TripleQuery::improve_spo_vec_iri_type_tag(
    std::shared_ptr<core::TripleSpec> p_triple) {
  for (size_t i = 0; i < 3; i++) {
    if (IRITypeUnionTag::UD == spo_vec_iri_type_tag_[i]) {
      if (core::IRIType::is_hashvalue(
              p_triple->at(static_cast<core::TripleElemPos>(i)).c_str(),
              p_triple->at(static_cast<core::TripleElemPos>(i)).size())) {
...
void TripleQuery::select_new_triple(
  ...
  for (auto iter = var_pos_.begin(); iter != var_pos_.end(); iter++) {
    ...
    if (IRITypeUnionTag::HV == var_tag) {
      ...
      select_spo_vec_[static_cast<int>(var_pos)].push_back(
          p_triple_spec->at(var_pos));
    } else {
...
std::string TripleQuery::update_cartesian_product_in_shared_data(
    std::string var_name) {
  //进入这一步的肯定有且只有两个var
  ...
  if (another_tag == IRITypeUnionTag::HV && var_tag == IRITypeUnionTag::HV) {
    for (size_t i = 0; i < select_spo_vec_[static_cast<int>(var_pos)].size();
         i++) {
      auto another_val =
          boost::get<HV_T>(select_spo_vec_[static_cast<int>(another_pos)][i]);
      auto var_val =
          boost::get<HV_T>(select_spo_vec_[static_cast<int>(var_pos)][i]);
  ...
  } else if (another_tag == IRITypeUnionTag::HV &&
             var_tag == IRITypeUnionTag::SS) {
    for (size_t i = 0; i < select_spo_vec_[static_cast<int>(var_pos)].size();
         i++) {
      auto another_val =
          boost::get<HV_T>(select_spo_vec_[static_cast<int>(another_pos)][i]);
      auto var_val =
          boost::get<SS_T>(select_spo_vec_[static_cast<int>(var_pos)][i]);
      if (p_shared_->hv_bound_vals_[another_var_name].find(another_val) ==
          p_shared_->hv_bound_vals_[another_var_name].end()) {
        continue;
      }
  ...
  } else if (another_tag == IRITypeUnionTag::SS &&
             var_tag == IRITypeUnionTag::SS) {
    for (size_t i = 0; i < select_spo_vec_[static_cast<int>(var_pos)].size();
         i++) {
      auto another_val =
          boost::get<SS_T>(select_spo_vec_[static_cast<int>(another_pos)][i]);
      auto var_val =
          boost::get<SS_T>(select_spo_vec_[static_cast<int>(var_pos)][i]);
  ...
  p_shared_->make_cartesian_product_by_filter_vector_linked(
      another_var_name, var_name, filter_var_val_vector);
...
void TripleQuery::select() {
  ...
  for (auto var_name : var_vec_) {
    if (p_shared_->var_val_type_.find(var_name) ==
        p_shared_->var_val_type_.end()) {
...
void TripleQuery::shrink_cur_valid_row_bm() {
  ...
  for (auto var_name : var_vec_) {
    ...
    BitMap_T union_bm;
    auto var_tag = spo_vec_iri_type_tag_[static_cast<int>(var_pos)];
    LOG("shrink_cur_valid_row_bm var pos :%d var_tag :%d",
        static_cast<int>(var_pos), static_cast<int>(var_tag));
    ...
    if (var_tag == IRITypeUnionTag::HV) {
      ...
      for (auto var_set_elem : var_set) {
        ...
        auto get_bm = p_fish_db->hv_meta_iri_index_.get_IRI_index(var_set_elem)
                          ->at(var_pos);
      ...
      for (auto var_set_elem : var_set) {
        union_bm |= p_fish_db->ss_meta_iri_index_.get_IRI_index(var_set_elem)
                        ->at(var_pos);
      }
    } else {
...
std::set<std::shared_ptr<TripleQuery>> make_triple_query_set(
    std::string sparql_query_str,
    std::shared_ptr<SharedQueryData> p_shared_data) {
  ...
  for (auto triple_it = triple_patterns.begin();
       triple_it != triple_patterns.end(); triple_it++) {
    ...
    LOG("triple tag %d %d %d ", static_cast<int>(triple_tag[0]),
        static_cast<int>(triple_tag[1]), static_cast<int>(triple_tag[2]));
    ...
    if (triple_tag[0] == SPARQLParserTypeTag::HV) {
      if (triple_tag[1] == SPARQLParserTypeTag::SS) {
        if (triple_tag[2] == SPARQLParserTypeTag::VAR) {
          auto triple_query = std::make_shared<TripleQuery>(
              ...
              boost::get<Variable>(triple[2]), p_shared_data);
          ret_query_set.insert(triple_query);
        }
      } else if (triple_tag[1] == SPARQLParserTypeTag::VAR) {
        if (triple_tag[2] == SPARQLParserTypeTag::HV) {
          auto triple_query = std::make_shared<TripleQuery>(
              boost::get<HashValue>(triple[0]), boost::get<Variable>(triple[1]),
              boost::get<HashValue>(triple[2]), p_shared_data);
          ret_query_set.insert(triple_query);
        } else if (triple_tag[2] == SPARQLParserTypeTag::SS) {
          auto triple_query = std::make_shared<TripleQuery>(
              boost::get<HashValue>(triple[0]), boost::get<Variable>(triple[1]),
              boost::get<ShortString>(triple[2]), p_shared_data);
          ret_query_set.insert(triple_query);
      ...
      if (triple_tag[1] == SPARQLParserTypeTag::SS) {
        ...
        if (triple_tag[2] == SPARQLParserTypeTag::HV) {
          auto triple_query = std::make_shared<TripleQuery>(
              boost::get<ShortString>(triple[0]),
              boost::get<Variable>(triple[1]), boost::get<HashValue>(triple[2]),
              p_shared_data);
          ret_query_set.insert(triple_query);
          ...
          auto triple_query = std::make_shared<TripleQuery>(
              boost::get<ShortString>(triple[0]),
              boost::get<Variable>(triple[1]), boost::get<Variable>(triple[2]),
              p_shared_data);
          ret_query_set.insert(triple_query);
        }
      }
      ...
      if (triple_tag[1] == SPARQLParserTypeTag::SS) {
        ...
        if (triple_tag[2] == SPARQLParserTypeTag::HV) {
          auto triple_query = std::make_shared<TripleQuery>(
              boost::get<Variable>(triple[0]), boost::get<Variable>(triple[1]),
              boost::get<HashValue>(triple[2]), p_shared_data);
          ret_query_set.insert(triple_query);
        } else if (triple_tag[2] == SPARQLParserTypeTag::SS) {
          auto triple_query = std::make_shared<TripleQuery>(
