...
enum class SPARQLParserTypeTag { HV = 0, SS, VAR };
...
class SharedQueryData {
 ...
 public:
  SharedQueryData(std::shared_ptr<fishdb::FishDBImpl> p_fish_db)
      : p_fish_db_(p_fish_db) {}
  ...
  void set_need_output_vars(std::vector<std::string>& vec) {
    need_output_vars_ = vec;
  }
  // debug
 ...
 private:
  ...
  std::unordered_map<std::string, std::set<HV_T>>
      hv_bound_vals_;  // var_1 -> set(val_1,val_2,...)
...
class TripleQuery {
 ...
 public:
  TripleQuery(HashValue sub, ShortString pre, Variable obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(HashValue sub, Variable pre, HashValue obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(HashValue sub, Variable pre, ShortString obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(HashValue sub, Variable pre, Variable obj,
              std::shared_ptr<SharedQueryData> p_shared);

  TripleQuery(ShortString sub, ShortString pre, Variable obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(ShortString sub, Variable pre, HashValue obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(ShortString sub, Variable pre, ShortString obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(ShortString sub, Variable pre, Variable obj,
              std::shared_ptr<SharedQueryData> p_shared);

  TripleQuery(Variable sub, ShortString pre, HashValue obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(Variable sub, ShortString pre, ShortString obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(Variable sub, ShortString pre, Variable obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(Variable sub, Variable pre, HashValue obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(Variable sub, Variable pre, ShortString obj,
              std::shared_ptr<SharedQueryData> p_shared);
  TripleQuery(Variable sub, Variable pre, Variable obj,
              std::shared_ptr<SharedQueryData> p_shared);
 ...
 private:
  ...
  int get_iri_index(IRITypeUnion_T iri_variant, IRITypeUnionTag typetag,
                    core::TripleElemPos pos, BitMap_T& ret_bitmap);
 ...
 private:
  ...
  std::unordered_map<std::string, core::TripleElemPos>
      var_pos_;  // var-pos ( var_name-> pos )
  IRITypeUnionTag spo_vec_iri_type_tag_
      [3];  // s-p-o iri_type [ note var's val has type ! ]
            // //其实这里与type-system是冲突的(当pre不同时）
