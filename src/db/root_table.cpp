...
void RootTable::dump(const std::string& root_table_file) {
  ...
  size_t prtable_spec_flexible_size =
      sizeof(RootTableSpec) +
      sizeof(RootTableSpec::data_type) * (block_offset_list_.size() - 1);
  RootTableSpec* prtable_spec =
      (RootTableSpec*)malloc(prtable_spec_flexible_size);
...
void RootTable::fast_init_by_dump_file(const std::string& root_table_file) {
  ...
  RootTableSpec* prtable_spec =
      (RootTableSpec*)mmap(nullptr, len, PROT_READ, MAP_SHARED, fd, 0);
...
void RootTable::modify_block_index_at(
    uint64_t block_id, uint64_t block_offset,
    std::shared_ptr<BlockIndex> p_block_index) {
  block_offset_list_[block_id] = block_offset;
  block_index_list_[block_id] = p_block_index;
}

bool RootTable::append_block_index(uint64_t block_offset,
                                   std::shared_ptr<BlockIndex> p_block_index) {
...
int RootTable::get_seek_pos_by_row_index(uint64_t row_index,
                                         uint32_t& ret_block_id,
                                         uint64_t& ret_block_offset) {
  ret_block_id = get_block_index_by_global_offset(row_index);
  LOG("get ret_block_id : %d, block_index_list_ size : %d", ret_block_id,
      block_index_list_.size());
