...
namespace test {
class RootTableTest; /* forward-declaration */
}  // namespace test
...
struct RootTableSpec {
 ...
 public:
  size_t get_flexible_size() {
    return sizeof(RootTableSpec) + sizeof(data_type) * (list_size_ - 1);
  }
...
class RootTable {
 ...
 public:
  ...
  void modify_block_index_at(uint64_t block_id, uint64_t block_offset,
                             std::shared_ptr<BlockIndex> p_block_index);
  bool append_block_index(uint64_t block_offset,
                          std::shared_ptr<BlockIndex> p_block_index);
