...
class AsioGenericServer {
 public:
  AsioGenericServer(std::shared_ptr<fishdb::FishDBImpl> p_fishdb,
 ...
 private:
  void handler_new_connection(std::shared_ptr<ConnectionHandler> handler,
                              const boost::system::error_code& error) {
    LOG("hello");
    if (error) {
      return;
    ...
    auto new_handler =
        std::make_shared<ConnectionHandler>(io_service_, p_fishdb_);
    LOG("hello");
