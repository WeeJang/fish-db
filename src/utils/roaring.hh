/* auto-generated on 2017年 3月12日 星期日 16时18分45秒 CST. Do not edit! */
#include "roaring.h"
/* begin file
 * /Users/jiangwei/Workspace/cayley-backend/src/lib/CRoaring/cpp/roaring.hh */
/*
 ...
 public:
  ...
  Roaring(const Roaring &r) {
    bool is_ok = ra_copy(&r.roaring.high_low_container,
                         &roaring.high_low_container, r.roaring.copy_on_write);
    if (!is_ok) {
      throw std::runtime_error("failed memory alloc in constructor");
    }
 ...
 * /Users/jiangwei/Workspace/cayley-backend/src/lib/CRoaring/cpp/roaring.hh */
/* begin file
 * /Users/jiangwei/Workspace/cayley-backend/src/lib/CRoaring/cpp/roaring64map.hh
 */
/*
...
class Roaring64Map {
 public:
  ...
  bool isFull() const {
    ...
    return roarings.size() == ((size_t)std::numeric_limits<uint32_t>::max()) + 1
               ? std::all_of(
                     ...
                     [](const std::pair<uint32_t, Roaring> &map_entry) {
                       // roarings within map are saturated if cardinality is
                       // uint32_t max + 1
  ...
  bool operator==(const Roaring64Map &r) const {
    ...
    do {
      // if the left map has reached its end, ensure that the right map contains
      // only empty Bitmaps
      ...
      do {
        // if the right map has reached its end, ensure that the right map
      ...
      // if neither map has reached its end ensure elements are equal and move
      // to the next element in both
    ...
    std::for_each(
        ...
        [&buf, portable](const std::pair<uint32_t, Roaring> &map_entry) {
          ...
          *((uint32_t *)buf) = map_entry.first;
          ...
          // TODO: lower-level read API should return bytes read so we don't
          // have to store this
  ...
  static Roaring64Map read(const char *buf, bool portable = true) {
    ...
    for (uint64_t lcv = 0; lcv < map_size; lcv++) {
      ...
      // TODO: lower-level read API should return bytes read so we don't have to
      // store this
      buf += bytesInRoaring;
    }
    return result;
   ...
   */
  size_t getSizeInBytes(bool portable = true) const {
    // start with, respectively, map size and for each map entry, size of keys
    // and Roaring serialized bytes
    return std::accumulate(
        roarings.cbegin(), roarings.cend(),
        sizeof(uint32_t) +
            roarings.size() * (sizeof(uint32_t) + sizeof(size_t)),
        [=](uint64_t previous, const std::pair<uint32_t, Roaring> &map_entry) {
    ...
    if (!isEmpty()) {
      ...
      map_iter->second.iterate(
          [](uint32_t low_bits, void *inner_iter_data) -> bool {
            std::printf(
                "%c%llu", ((iter_data *)inner_iter_data)->first_char,
