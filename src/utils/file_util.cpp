...
std::vector<std::string> get_file_list_with_prefix(
    const std::string& dictionary_path, const std::string& prefix) {
  std::vector<std::string> all_files = get_file_list(dictionary_path);
  std::vector<std::string> ret_files(all_files.size());

  auto iter_end = std::copy_if(
      all_files.begin(), all_files.end(), ret_files.begin(),
      [&prefix](std::string elem) {
        return (0 == ::strncmp(elem.c_str(), prefix.c_str(), prefix.size()));
      });
...
bool write_to_file(const std::string& dump_file_path, char* serialized_bytes,
  ...
  int fd = -1;
  LOG("dump file path : %s , serialized_bytes : %c size_t :%zu ",
      dump_file_path.c_str(), serialized_bytes, expected_size);
