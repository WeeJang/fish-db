...
int FishDBImpl::open_db(const std::string& db_name) {
  ...
  std::vector<std::string> block_file_list =
      utils::get_file_list(db_block_dir_path_);
  ...
  for (auto& file_name : block_file_list) {
    ...
    root_table_.modify_block_index_at(p_block_index->block_id(),
                                      p_block_index->row_start_index(),
                                      p_block_index);
  }
...
int FishDBImpl::create_db(const std::string& db_name) {
  ...
  if (-1 == utils::mkdir(db_dir_path_)) {
    fprintf(stderr, "mkdir %s failed! Maybe exist alreday .",
            db_dir_path_.c_str());
    exit(-1);
  }
  if (-1 == utils::mkdir(db_block_dir_path_)) {
    fprintf(stderr, "mkdir %s failed! Maybe exist alreday .",
            db_block_dir_path_.c_str());
    exit(-1);
  }
  if (-1 == utils::mkdir(db_iri_dir_path_)) {
    fprintf(stderr, "mkdir %s failed! Maybe exist alreday .",
            db_iri_dir_path_.c_str());
    exit(-1);
  }
...
int FishDBImpl::load_data(const std::string& triple_file_path) {
  ...
  while ((has_read_size = ::read(fd, buffer, block_size)) != -1) {
    ...
    for (size_t i = 0; i < row_block_size; i++) {
      ...
      iri_index->add_index(triple_elem_pos++, row_offset_in_global);
      LOG("add result triple_pos : %hhd, row_offset_in_global : %llu",
          triple_elem_pos, row_offset_in_global);
    }
...
std::shared_ptr<db::BlockDataSeeker> FishDBImpl::get_block_data_seeker(
    size_t block_id) {
  bool is_exist = false;
  {
  	boost::shared_lock<boost::shared_mutex> sl(block_data_seeker_map_mutex_);
        is_exist = check_data_seeker(block_id);	      
  }
  if(!is_exist){ load_data_seeker(block_id); }
  return block_data_seeker_map_.at(block_id);
  }

void FishDBImpl::load_data_seeker(size_t block_id){
    boost::unique_lock<boost::shared_mutex> ul(block_data_seeker_map_mutex_);
    if(check_data_seeker(block_id)) { return ;}
    std::string block_path(db_block_dir_path_);
...
bool FishDBImpl::check_data_seeker(size_t block_id){
...
int FishDBImpl::get_triple_by_row_index(
    uint64_t row_index, std::shared_ptr<core::TripleSpec> p_triple_spec) {
  uint32_t block_id;
  uint64_t block_offset;

  LOG("row index : %llu", row_index);
  if (-1 == root_table_.get_seek_pos_by_row_index(row_index, block_id,
                                                  block_offset)) {
