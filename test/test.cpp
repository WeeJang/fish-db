...
uint64_t get_block_index_by_global_offset(
    const std::vector<uint64_t> block_offset_list_, uint64_t global_offset) {
...
int main(int argc, char** argv) {
...

  {
          int* p = new int(0);
          int m = 4;
          std::unique_ptr<int,std::function<void(int*)>> a(p,[m](int* D){ delete
  D ; std::cout << m << " hello world " << std::endl; } ); std::cout << *a <<
  std::endl;
  }
